    ...
    return (
        <>
            <Space wrap className="p-4 bg-pink-100 rounded-2xl shadow-inner-soft flex justify-around items-center w-full">
                <Button 
                    block shape='rounded' size='middle' 
                    className={addType === ADD_TYPE_VALUE.ADD_INCOME ? "text-pink-500 ring-2 ring-pink-400" : ""} 
                    onClick={() => handleSolve(ADD_TYPE_VALUE.ADD_INCOME)}>
                    {ADD_TYPE.ADD_INCOME}
                </Button>
                <Button 
                    block shape='rounded' size='middle'
                    className={addType === ADD_TYPE_VALUE.ADD_EXPENSE ? "text-pink-500 ring-2 ring-pink-400" : ""} 
                    onClick={() => handleSolve(ADD_TYPE_VALUE.ADD_EXPENSE)}>
                    {ADD_TYPE.ADD_EXPENSE}
                </Button>
                <Button
                    block shape='rounded' size='middle'
                    className={addType === ADD_TYPE_VALUE.ADD_INSIGHT ? "text-pink-500 ring-2 ring-pink-400" : ""}
                    onClick={() => handleSolve(ADD_TYPE_VALUE.ADD_INSIGHT)}>
                    {ADD_TYPE.ADD_INSIGHT}
                </Button>
            </Space>
            <ModalItem
                showModal={showModal}
